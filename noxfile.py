try:
    ...
    def build(session):
        ...
        env=dict(
            SOURCE_DATE_EPOCH=subprocess.check_output("git log -1 --format=%ct".split()).decode("utf-8").strip()
        )
    ...
    def test(session):
        ...
        session.run(*"pytest -vv --pyargs . --nbval -pno:importnb --durations=5 --cov=schemata --cov-report=term-missing:skip-covered --no-cov-on-fail".split())
