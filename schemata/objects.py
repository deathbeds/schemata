...
from .types import ANNO, EMPTY, Any, Const, Schemata, Type
...
class Dict(Type["object"], mixins.Context, dict):
    def __class_getitem__(cls, object):
        ...
        return builders.InstanceBuilder(**build_dict(object)).build() + cls

    @classmethod
    def properties(cls, *args, **kwargs):
        return Properties[dict(*args, **kwargs)] + cls
...
class Properties(Any.from_key("properties")):
    @validates(dict)
...
class AdditionalProperties(Any.from_key("additionalProperties")):
    @validates(dict)
    def validator(cls, object):
        pass
...
class Required(Any.from_key("required")):
    @validates(dict)
    def validator(cls, object):
...
class PatternProperties(Any.from_key("patternProperties")):
    @validates(dict)
    def validator(cls, object):
...
class PropertyNames(Any.from_key("propertyNames")):
    @validates(dict)
    def validator(cls, object):
...
class MinProperties(Any.from_key("minProperties")):
    @validates(dict)
    def validator(cls, object):
        testing.assertGreaterEqual(len(object), cls.value(MinProperties))


class MaxProperties(Any.from_key("maxProperties")):
    @validates(dict)
    def validator(cls, object):
        testing.assertLessEqual(len(object), cls.value(MaxProperties))


class Dependencies(Any.from_key("dependencies")):
    @validates(dict)
    def validator(cls, object):
